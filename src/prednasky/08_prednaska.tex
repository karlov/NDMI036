...
\begin{lemma}[Sjednocení podprostorů]\label{kpp:sub_join}
	...
	\begin{enumerate}
	 ...
	 Podle \cref{kpp:kolin_centr_kolin} je tato kolineace centrální, takže Col(H) je grupa.
		...
		\item Za prvé, $T(H)$ je grupa. Nechť $\alpha, \beta \ne id \in T(H)$.
			...
			\begin{enumerate}
				\item $C \ne D$. Body $D, X, \beta(X)$ jsou kolineární, pak i $\beta(D) = D, \alpha(X), \beta(\alpha(X))$.
...
\begin{lemma}[Kolineace jsou automorfismy grupy]
	...
	\begin{enumerate}
		\item $\alpha(O)=O$
		\item $\forall X,Y \alpha(X+Y)=\alpha(X)+\alpha(Y)$
		\item $\forall X \in P^*: \alpha(-X)=-\alpha(X)$.
	\end{enumerate}
\end{lemma}
\begin{proof}~
	%\begin{enumerate}
